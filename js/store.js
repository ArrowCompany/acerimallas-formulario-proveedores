...
function mapEquipo(row) {
  return { id: row[0], proveedorId: row[1], nombre: row[2], ubicacion: row[3], serie: row[4], proximoMantenimiento: row[5] };
}
...
async function getProveedores() {
  const res = await fetch(`${WEBAPP_URL}?accion=listarProveedores`);
...
async function guardarProveedor(datos) {
  const res = await fetch(WEBAPP_URL, {
    method: 'POST',
    body: JSON.stringify({ accion: 'guardarProveedor', datos })
  });
  return res.json();
}

async function actualizarEstadoProveedor(id, estado, camposConError) {
  const res = await fetch(WEBAPP_URL, {
    method: 'POST',
    body: JSON.stringify({ accion: 'actualizarEstadoProveedor', id, estado, camposConError })
  });
  return res.json();
}
...
async function getEquipos(proveedorId) {
  const url = proveedorId ? `${WEBAPP_URL}?accion=listarEquipos&proveedorId=${proveedorId}` : `${WEBAPP_URL}?accion=listarEquipos`;
...
async function agregarEquipo(datos) {
  const res = await fetch(WEBAPP_URL, {
    method: 'POST',
    body: JSON.stringify({ accion: 'agregarEquipo', datos })
  });
  return res.json();
}
...
async function getMantenimientos(equipoId) {
  const url = equipoId ? `${WEBAPP_URL}?accion=listarMantenimientos&equipoId=${equipoId}` : `${WEBAPP_URL}?accion=listarMantenimientos`;
  const res = await fetch(url);
  const data = await res.json();
  if (!data.ok) { console.error(data.error); return []; }
...
async function agregarMantenimiento(datos) {
  const res = await fetch(WEBAPP_URL, {
    method: 'POST',
    body: JSON.stringify({ accion: 'agregarMantenimiento', datos })
  });
  return res.json();
}
...
async function enviarCorreoPruebaBackend(correos) {
  const res = await fetch(WEBAPP_URL, {
    method: 'POST',
    body: JSON.stringify({ accion: 'enviarCorreoPrueba', correos })
  });
  return res.json();
}
